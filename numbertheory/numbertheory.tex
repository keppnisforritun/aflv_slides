...
\begin{frame}[plain]{Definitions that everybody should know}
  ...
  \begin{itemize}
    ...
    \item {\color{red}Least Common Multiple} of two integers $a$ and $b$ is
      the smallest positive integer that both $a$ and $b$ divide.
  \end{itemize}
\end{frame}
...
\begin{frame}[plain]{Extended Euclidean algorithm}
  \vspace{20pt}
  \begin{itemize}
    \item Reversing the steps of the Euclidean algorithm we get B\'ezout's identity
      \[
        \gcd(a, b) = ax + by
      \]
...
\begin{frame}[plain]{Factoring}
  ...
  \begin{itemize}
    \item Every integer greater than $1$ is a unique product of primes, up to order.
    \onslide<2->
    \[
...
\begin{small}
\begin{itemize}
    \item<1-> Let $g(x) = x^2 + 1 \ (mod \ n)$ and create the sequence $x_1 = x, x_2 = g(x), x_3 = g(g(x)), \dots$ where $x$ is chosen randomly.
    \item<2-> Check if $\operatorname{gcd}(x_i - x_j, n) > 1$.
    \item<3-> The sequence has begun repeating not just modulo $n$ but modulo $d$ where $d$ divides $n$.
    ...
    \item<5-> Test a few starting values of $x$ before giving up. Usually good for $n > 2^{32}$.
    \item<6-> The time complexity is an open question, but it's conjectured to be the square root of the largest factor of $N$.
    \item<7-> Slow for primes, but much faster for composite numbers.
    \item<8-> Checking for primality first using Miller-Rabin can be useful.
\end{itemize}
\end{small}
