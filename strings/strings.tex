...
\begin{frame}{Choosing a Hash Function}
    ...
    \begin{itemize}
        ...
        \item \alert{Modulus (m):} A \alert{large prime} or \alert{$2^{64}$} (using integer overflow).
    \end{itemize}
    ...
    \begin{example}
        ...
        $H($``abc''$) = (97 \cdot 131^0 + 98 \cdot 131^1 + 99 \cdot 131^2) \mod m$
    \end{example}
\end{frame}
...
\begin{frame}{The Power of Precomputation}
    ...
    \vspace{5pt}
    \textbf{Hash of substring}:
    \begin{align*}
        H(s[i..j]) = &\left(pref[j+1] - pref[i] \cdot pow[j - i + 1]\right) \pmod{m}
    \end{align*}
    \vspace{-5pt}
    \begin{block}{}
        This enables \alert{$\mathcal{O}(1)$} time extraction of any substring's hash after \alert{$\mathcal{O}(n)$} precomputation.
    \end{block}
\end{frame}
...
\begin{frame}{The Rolling Hash - Efficient Updates}
    ...
    \begin{enumerate}
        ...
        \item For each starting position $i$ in $T$:
            ...
            \end{itemize}
            \begin{align*}
                H_{\text{next}} = &\left( (H_{\text{old}} - T[i] \cdot b^{|P|-1}) \cdot b + T[i+|P|] \right) \pmod{m}
            \end{align*}
    \end{enumerate}
...
\begin{frame}{Extending the Idea: 2D \& Beyond}
    ...
    \begin{small}
    \begin{align*}
        H_{\text{2D}}(P) &= \sum_{i=0}^{h-1} H(\text{row } i) \cdot B^{i} \pmod{m} \\
        H_{\text{2D}}(T[x][y..y+w-1]) &= \sum_{i=0}^{h-1} H(T[x+i][y..y+w-1]) \cdot B^{i} \pmod{m}
    \end{align*}
    \end{small}
...
\begin{frame}{Implementation Tips and Pitfalls}
    ...
    \begin{itemize}
        \item \alert{Modulo Arithmetic:} Prevents overflow and slow arithmetic.
    ...
    \begin{itemize}
        ...
        \item Know your domain of strings.
        \item \alert{Tuple of hashes}, avoiding big integer arithmetic.
    \end{itemize}
\end{frame}
...
\begin{frame}{Conclusion and Further Reading}
    ...
    \begin{itemize}
        ...
        \item It is \alert{probabilistic} but can be made very reliable with good parameters and double hashing.
    \end{itemize}
