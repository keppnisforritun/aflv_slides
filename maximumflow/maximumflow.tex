...
\begin{document}
	...
	\begin{frame}[plain]{Example}
		\begin{center}
			\begin{tikzpicture}[scale=1.2]
				...
				\draw[very thick, ->]  (5) -- (3) node[midway,above right] () {$0/15$};
	...
	\begin{frame}[plain]{Example}
		\begin{center}
			\begin{tikzpicture}[scale=1.2]
				...
				\draw[very thick, ->]  (3) -- (6) node[midway,above] () {$8/9$};
				\draw[very thick, ->]  (5) -- (3) node[midway,above right] () {$3/15$};
				\draw[very thick, ->]  (4) -- (2) node[midway,above right] () {$3/6$};
	...
	\begin{frame}[plain]{Now what?}
		\begin{itemize}
			\item We're stuck! The max flow is not $4$, and there is no min cut.
...
\end{minted}
	...
	\begin{frame}[plain]{Bipartite matching}
		\begin{center}
			\begin{tikzpicture}[scale=1]
				...
				\draw[very thick, ->]  (6) -- (B) node[midway,below right] () {$1$};
				\draw[very thick, ->]  (7) -- (B) node[midway,above left] () {$1$};
				\draw[very thick, ->]  (8) -- (B) node[midway,above] () {$1$};
				\draw[very thick, ->]  (9) -- (B) node[midway,below left] () {$1$};
				\draw[very thick, ->]  (A) -- (B) node[midway,above right] () {$1$};
			\end{tikzpicture}
		\end{center}
	\end{frame}
	...
	\begin{center}
		\begin{tikzpicture}[scale=1.2]
			...
			\draw[very thick, ->]  (6) -- (A) node[midway,below right] () {$w_k + g_k - w_z - 1$};
			\draw[very thick, ->]  (7) -- (A) node[midway,below left = 0.1 and -0.55] () {$w_k + g_k - w_b - 1$};
			\draw[very thick, ->]  (9) -- (A) node[midway,above right] () {$w_k + g_k - w_a - 1$};
...
long long dfs(int v, long long pushed) {
	...
	if (v == t)
	    return pushed;
	for (int& cid = ptr[v]; cid < adj[v].ssize(); cid++) {
		int id = adj[v][cid];
		int u = edges[id].u;
	...
	\begin{frame}[plain]{MCMF}
		\begin{itemize}
			...
			\item Note that the cost is calculated as cost per unit of flow, so sending 2 units of flow from $A$ to $B$ is always twice the cost of sending one unit
		\end{itemize}
	\end{frame}
